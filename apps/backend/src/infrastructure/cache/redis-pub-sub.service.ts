...
export class RedisPubSubService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      `📡 Subscribed to cache invalidation channel: ${this.channel}`,
    );
    ...
    this.logger.log(
      `🚀 Pub/Sub service initialized with instance ID: ${this.instanceId}`,
    );
  }
  ...
  async publishUserInvalidation(
    ...
    try {
      ...
      this.logger.debug(
        `📤 Published user invalidation: ${userId} in tenant ${tenantId}`,
      );
    } catch (error) {
  ...
  async publishTenantInvalidation(tenantId: string): Promise<void> {
    ...
    try {
      await this.publisher.publish(this.channel, JSON.stringify(message));
      this.logger.debug(
        `📤 Published tenant invalidation: ${tenantId}`,
      );
    } catch (error) {
