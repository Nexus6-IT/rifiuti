...
describe('SubscriptionEnforcementService', () => {
  ...
  describe('assertFirLimitNotReached', () => {
    ...
    it('lancia ForbiddenException se count >= limite mensile', async () => {
      ...
      await expect(service.assertFirLimitNotReached('tenant-1')).rejects.toThrow('Limite mensile FIR raggiunto');
    });
  ...
  describe('assertUserLimitNotReached', () => {
    it('non lancia se count < limite utenti', async () => {
      ...
      await expect(service.assertUserLimitNotReached('tenant-1', 'OPERATOR')).resolves.toBeUndefined();
    });
    ...
    it('lancia ForbiddenException se count >= limite utenti', async () => {
      ...
      await expect(service.assertUserLimitNotReached('tenant-1', 'OPERATOR')).rejects.toThrow(ForbiddenException);
    });
