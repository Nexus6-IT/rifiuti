...
export class SpidStrategy extends PassportStrategy(Strategy, 'spid') {
  constructor(
    ...
    const idpCert =
      configService.get<string>('SPID_IDP_CERT') ||
    ...
    super({
      ...
      entryPoint:
        configService.get<string>('SPID_IDP_ENTRY_POINT') ||
        configService.get<string>('SPID_ENTRY_POINT') ||
        'http://localhost:8080/samlsso',
      issuer:
        configService.get<string>('SPID_ENTITY_ID') ||
        configService.get<string>('SPID_ISSUER') ||
        'wasteflow-dev',
      callbackUrl:
        configService.get<string>('SPID_CALLBACK_URL') ||
  ...
  async validate(profile: SpidProfile): Promise<SpidUser> {
    ...
    const _firstName = profile.name
    const _lastName = profile.familyName
