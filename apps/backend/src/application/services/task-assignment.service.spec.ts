...
describe('TaskAssignmentService', () => {
  ...
  describe('findQualifiedDrivers', () => {
    it('should find drivers with matching certifications', async () => {
      ...
      resourceOwnershipRepository.findActiveByTenant.mockResolvedValue(
        mockDriverOwnerships as any,
      );
      firRepository.getCurrentWorkloadByDriver.mockResolvedValue(200);

      // Act
      const qualifiedDrivers = await service.findQualifiedDrivers(
        firId,
        mockTenantId,
      );
    ...
    it('should throw NotFoundException if FIR does not exist', async () => {
      ...
      // Act & Assert
      await expect(
        service.findQualifiedDrivers(firId, mockTenantId),
      ).rejects.toThrow(NotFoundException);
      await expect(
        service.findQualifiedDrivers(firId, mockTenantId),
      ).rejects.toThrow('FIR not found');
    });
  });
  ...
  describe('autoAssignTask', () => {
    it('should automatically assign FIR to best qualified driver', async () => {
      ...
      firRepository.findById.mockResolvedValue(mockFir as any);
      jest
        .spyOn(service, 'findQualifiedDrivers')
        .mockResolvedValue(mockQualifiedDrivers as any);
      firRepository.save.mockResolvedValue(mockFir as any);

      // Act
      const assignment = await service.autoAssignTask(
        firId,
        mockTenantId,
        mockFleetManagerId,
      );
      ...
      expect(mockFir.assignDriver).toHaveBeenCalledWith(
        'driver-2',
        mockFleetManagerId,
      );
    });
    ...
    it('should throw error if no qualified drivers available', async () => {
      ...
      // Act & Assert
      await expect(
        service.autoAssignTask(firId, mockTenantId, mockFleetManagerId),
  ...
  describe('manualAssignTask', () => {
    ...
    it('should warn but allow manual assignment to unqualified driver', async () => {
      ...
      // Assert
      expect(assignment.assignedDriverId).toBe(driverId);
      expect(assignment.warnings).toContain(
        'Driver lacks required certifications: ADR',
      );
      expect(assignment.assignmentMethod).toBe('manual');
    });
  ...
  describe('getDriverWorkload', () => {
    it('should calculate current driver workload', async () => {
      ...
      firRepository.getCurrentWorkloadByDriver.mockResolvedValue(
        currentWorkload,
      );
      ...
      expect(firRepository.getCurrentWorkloadByDriver).toHaveBeenCalledWith(
        driverId,
        mockTenantId,
      );
    });
