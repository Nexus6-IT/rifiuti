...
export class PermissionSyncQueueService {
  ...
  private async syncOperation(operation: QueuedOperation): Promise<void> {
    ...
    try {
      ...
      const _result = await this.executeRequest(operation).toPromise()
      ...
      if (operation.retryCount >= this.MAX_RETRIES) {
        ...
        console.log(
          `[SyncQueue] Retrying operation ${operation.id} in ${delay}ms (attempt ${operation.retryCount}/${this.MAX_RETRIES})`
        )
