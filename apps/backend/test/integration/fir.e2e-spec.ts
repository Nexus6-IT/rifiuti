...
describe('FIR API (e2e)', () => {
  ...
  describe('GET /api/v1/fir', () => {
    beforeEach(async () => {
      // Create test FIRs
      for (let i = 0; i < 25; i++) {
        await testSetup.getApp().get('PrismaService').fIR.create({
          data: {
            id: `test-fir-${i}`,
    ...
    it('should enforce tenant isolation', async () => {
      ...
      const otherTenantFir = response.body.data.find(
        (fir: any) => fir.id === 'other-tenant-fir',
      );
  ...
  describe('GET /api/v1/fir/:id', () => {
    ...
    beforeEach(async () => {
      testFir = await testSetup.getApp().get('PrismaService').fIR.create({
        data: {
          id: 'test-fir-detail',
          tenantId: testData.tenant.id,
