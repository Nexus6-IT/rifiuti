...
export class BillingService {
  ...
  private async handleSubscriptionDeleted(subscription: Stripe.Subscription): Promise<void> {
    ...
    this.logger.log(
      `Abbonamento cancellato: tenant=${tenant.id} → declassato a TRIAL/EXPIRED`,
    );
  }
  ...
  private async handlePaymentFailed(invoice: Stripe.Invoice): Promise<void> {
    ...
    this.logger.warn(
      `Pagamento fallito: tenant=${tenant.id} → SUSPENDED`,
    );
  }
  ...
  private async applySubscriptionUpdate(
    ...
    const tier = priceId ? (this.priceIds[priceId] ?? SubscriptionTier.PROFESSIONAL) : SubscriptionTier.PROFESSIONAL;
