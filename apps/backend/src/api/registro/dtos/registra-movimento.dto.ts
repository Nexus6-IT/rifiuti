...
export class RegistraMovimentoDto {
  @ApiProperty({ enum: ['CARICO', 'SCARICO'], description: 'Tipo di movimento (art. 190 D.Lgs 152/2006)' })
  @IsEnum(['CARICO', 'SCARICO'])
  type: 'CARICO' | 'SCARICO'

  @ApiProperty({ description: 'Data effettiva dell\'operazione di carico o scarico', example: '2026-06-29' })
  @IsDateString()
  movementDate: string
  ...
  @ApiProperty({ description: 'Codice CER (6 cifre con o senza spazi, es. "200301" o "20 03 01")', example: '20 03 01' })
  @IsString()
  @IsNotEmpty()
  @MaxLength(10)
  cerCode: string
  ...
  @ApiPropertyOptional({ description: 'Caratteristiche di pericolo HP (es. "HP4,HP14") — obbligatorio per rifiuti pericolosi' })
  @IsString()
  @IsOptional()
  @MaxLength(100)
  ...
  @ApiPropertyOptional({ description: 'Denominazione della controparte (cedente per CARICO, destinatario per SCARICO)' })
  @IsString()
  @IsOptional()
  @MaxLength(255)
  ...
  @ApiPropertyOptional({ description: 'ID del FIR collegato (obbligatorio per SCARICO con causale CONFERIMENTO_TRASPORTATORE)' })
  @IsUUID()
  @IsOptional()
  firId?: string
