...
export class RENTRISyncService {
  ...
  syncFIR(
    ...
    return this.http.post<SyncFIRResponse>(`${this.baseUrl}/fir/${firId}`, {
  ...
  batchSync(
    ...
    return this.http.post<BatchSyncResponse>(`${this.baseUrl}/batch`, {
      firIds,
      priority: options?.priority || 'normal',
      correlationId: options?.correlationId,
    })
   ...
   */
  pollSyncStatus(jobId: string, pollIntervalMs: number = 2000): Observable<SyncJobStatus> {
  ...
  isSyncCompleted(jobId: string): Observable<boolean> {
    return this.getSyncStatus(jobId).pipe(map(status => status.status === 'completed'))
  }
  ...
  isSyncFailed(jobId: string): Observable<boolean> {
    return this.getSyncStatus(jobId).pipe(map(status => status.status === 'failed'))
  }
}
