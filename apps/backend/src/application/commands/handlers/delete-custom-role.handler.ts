...
@Injectable()
export class DeleteCustomRoleCommandHandler implements ICommandHandler<DeleteCustomRoleCommand> {
  ...
  async execute(command: DeleteCustomRoleCommand): Promise<void> {
    try {
      this.logger.log(`Attempting to delete role ${command.roleId} for tenant ${command.tenantId}`)
      ...
      this.logger.log(`✓ Deleted custom role "${role.name}" for tenant ${command.tenantId}`)
    } catch (error) {
      if (
      ...
      this.logger.error(`Failed to delete custom role: ${error.message}`, error.stack)
