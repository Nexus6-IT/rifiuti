...
export class PdfGenerationJobProcessor extends WorkerHost {
  ...
  async process(job: Job<PdfGenerationJobData>): Promise<PdfGenerationJobResult> {
    ...
    for (const documentId of documentIds) {
      try {
        ...
        const pdfData = await this.generateDocumentPdf(tenantId, documentType, documentId, options)
  ...
  private async generateDocumentPdf(
    ...
    documentType: 'fir' | 'mud',
    documentId: string,
    _options?: PdfGenerationJobData['options']
  ...
  private async combinePdfsIntoBatch(
    ...
    const pdfBuffers = await Promise.all(pdfs.map(pdf => this.s3Service.downloadFile(pdf.s3Key)))
