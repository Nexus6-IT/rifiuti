...
describe('TemporaryPermissionGrant', () => {
  describe('Creation', () => {
    it('should create grant with required fields', () => {
      ...
      expect(grant.justification).toBe('Quarterly audit - need export access for compliance reporting');
  ...
  describe('Revocation', () => {
    ...
    it('should throw error if revoking non-approved grant', () => {
      ...
      // Act & Assert
      expect(() => grant.revoke('admin-789', 'Revoked')).toThrow(
        'Can only revoke approved grants',
      );
    });
