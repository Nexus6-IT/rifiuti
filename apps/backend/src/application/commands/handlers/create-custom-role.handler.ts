...
@Injectable()
export class CreateCustomRoleCommandHandler implements ICommandHandler<CreateCustomRoleCommand> {
  ...
  async execute(command: CreateCustomRoleCommand): Promise<Role> {
    try {
      this.logger.log(`Creating custom role "${command.name}" for tenant ${command.tenantId}`)

      // Step 1: Validate role name
      this.validateRoleName(command.name)

      // Step 2: Check for duplicate role name in tenant
      const existingRole = await this.roleRepository.findByName(command.name, command.tenantId)
      ...
      this.logger.error(`Failed to create custom role: ${error.message}`, error.stack)
