...
export const TempPermissionStore = signalStore(
  ...
  withComputed((store) => ({
    ...
    effectivePermissions: computed(() => {
      const active = store
        .grants()
        .filter((grant) => grant.isActive && !grant.revokedAt);
  ...
  withMethods((store, http = inject(HttpClient)) => ({
    ...
    loadGrants: rxMethod<void>(
      pipe(
        ...
        switchMap(() =>
          http
            .get<{ grants: TempPermissionGrant[] }>('/api/v1/temp-permissions/me')
            .pipe(
              tapResponse({
                next: (response) => {
                  ...
                  const enrichedGrants = response.grants.map((grant) => {
                    const endTime = new Date(grant.endTime);
                    const isActive =
                      !grant.revokedAt && endTime.getTime() > now.getTime();
    ...
    requestGrant: rxMethod<{
      ...
      pipe(
        ...
        switchMap((request) =>
          http
            .post<{ grant: TempPermissionGrant }>(
              '/api/v1/temp-permissions/request',
              request,
            )
            .pipe(
              tapResponse({
