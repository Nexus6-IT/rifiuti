...
export const RoleStore = signalStore(
  ...
  withComputed(store => ({
     ...
     * Get role by ID
     */
    getRoleById: computed(() => (roleId: string): Role | undefined => {
     ...
     * Get role by name
     */
    getRoleByName: computed(() => (roleName: string): Role | undefined => {
      return store.roles().find(r => r.name.toLowerCase() === roleName.toLowerCase())
    }),
     ...
     */
    hasRole: computed(() => (roleName: string): boolean => {
      return store.userRoles().some(ur => ur.roleName?.toLowerCase() === roleName.toLowerCase())
    }),
    ...
    primaryRole: computed(() => {
      ...
      const activeRoles = store
        .userRoles()
        .filter(ur => !ur.expiresAt || new Date(ur.expiresAt).getTime() > now.getTime())
