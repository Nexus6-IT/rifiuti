...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, DropdownModule, ButtonModule, BadgeModule, TooltipModule],
  template: `
...
export class TenantSelectorComponent implements OnInit {
  ...
  private switchTenant(targetTenantId: string): void {
    ...
    this.http
      .post<any>('/api/v1/consultant/switch-tenant', {
        targetTenantId,
      })
      .subscribe({
        next: response => {
   ...
   */
  private getRoleDisplayName(_roleId: string): string {
