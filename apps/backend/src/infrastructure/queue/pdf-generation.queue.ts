...
export class PdfGenerationQueueService {
  constructor(
    @InjectQueue('pdf-generation') private readonly pdfQueue: Queue<PdfGenerationJobData, PdfGenerationJobResult>,
  ...
  async queueSinglePdf(params: {
    ...
    this.logger.info(`Queued single PDF generation: jobId=${job.id}, documentId=${params.documentId}`);
  ...
  async clearOldJobs(olderThanHours: number = 24): Promise<number> {
    ...
    this.logger.info(`Cleaned ${totalCleaned} old PDF jobs (completed: ${completed.length}, failed: ${failed.length})`);
