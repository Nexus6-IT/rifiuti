...
@Injectable()
export class UpdateCustomRoleCommandHandler
  implements ICommandHandler<UpdateCustomRoleCommand>
{
  ...
  async execute(command: UpdateCustomRoleCommand): Promise<Role> {
    try {
      this.logger.log(
        `Updating custom role ${command.roleId} for tenant ${command.tenantId}`,
      );
      ...
      await this.redisPubSub.publishRoleInvalidation(
        command.tenantId,
        command.roleId,
      );

      this.logger.log(
        `✓ Updated custom role "${updatedRole.name}" - cache invalidation completed`,
      );
      ...
      this.logger.error(
        `Failed to update custom role: ${error.message}`,
        error.stack,
      );
