 ...
 */
@Injectable()
export class PrismaResourceOwnershipRepository implements ResourceOwnershipRepository {
  ...
  async findById(id: string, tenantId: string): Promise<ResourceOwnership | null> {
  ...
  async findActiveByTenant(tenantId: string, resourceType?: string): Promise<ResourceOwnership[]> {
    const where: any = {
      tenantId,
      isActive: true,
  ...
  async findActiveVehicleAssignments(
    ...
    if (requiredCertifications && requiredCertifications.length > 0) {
      ownerships = ownerships.filter((o: any) => {
        ...
        return requiredCertifications.every(reqCert => driverCerts.includes(reqCert))
      })
    }
  ...
  async deactivateAllForUser(
    ...
    this.logger.log(`Deactivated ${result.count} assignments for user ${userId}`)
  ...
  async deactivateAllForResource(
    ...
    this.logger.log(`Deactivated ${result.count} assignments for resource ${resourceId}`)
